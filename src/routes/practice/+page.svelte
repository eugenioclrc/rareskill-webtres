...
<div class="w-full pt-0 pb-16">
    ...
    <div class="bg-white container max-w-4xl mt-8 mx-auto px-16 py-8 rounded-lg shadow-2xl">
        ...
        {/if}
            ...
            {#if showAnswerButton}
                <button type="button" on:click={renderAnswer} disabled={showAnswer} class:danger-btn={!showAnswer} class="base-btn">Show answer</button>
            {/if}

            {#if nextQuestion}
                <button type="button" on:click={gotoNextQuestion} class="base-btn success-btn">Next Question</button>
            {/if}
    </div>
</div>
